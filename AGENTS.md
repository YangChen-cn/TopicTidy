...
- `Sources/TopicTidyCore/Scanner` owns top-level enumeration across configured scan roots, fingerprints, cache lookup, provenance metadata, and persistence. It may depend on the `ExtractorRegistry` interface but must not import PDFKit or OOXML specifics.
...
- Scan roots default to Downloads and may be configured as a nonempty list of real, non-overlapping folders on the destination's volume. A plan retains its source roots as well as its destination; changing either preference must not redirect apply or undo. Changing scan roots turns off persisted auto-confirm until the user explicitly enables it again.
...
- Signing identity resolution lives in `scripts/lib/identity.sh`: `--identity`, then the `TopicTidy` certificate imported in CI from the `CERTIFICATE_P12`/`CERTIFICATE_PASSWORD` secrets, then ad-hoc for local runs. Never commit the certificate; it must stay in repository secrets.
- A tagged release must never ship ad-hoc signed. The release job fails when `CERTIFICATE_P12` is missing, verifies the identity by signing a throwaway file, and asserts `Authority=TopicTidy` on the packaged app before publishing.
- The same workflow updates the Homebrew tap in a follow-up `tap` job (`scripts/publish_tap.sh` with `TAP_GITHUB_TOKEN`), so a published version and the tap cannot drift apart. The job fails loudly when the token is absent rather than leaving the tap behind.
