...
NOISE = STOPWORDS | {
    ...
    "solution", "solutions", "tutorial", "lab", "material", "materials",
}
...
def _format_phrase(tokens: tuple[str, ...]) -> str:
    words = [token.upper() if token.isupper() else token.title() for token in tokens]
...
def display_name(files: list[IndexedFile], course_code: str = "") -> str:
    ...
    if stem_lists:
        ...
        ordered = tuple(token for token in stem_lists[0] if token in common)[:4]
        if ordered:
            return _format_phrase(ordered)

    informative_titles = [tokens for tokens in (_clean_tokens(file.title) for file in files) if tokens]
