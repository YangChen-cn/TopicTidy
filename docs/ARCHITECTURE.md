...
`Scanner` 负责已配置扫描目录的顶层枚举、稳定性检查、SHA-256 指纹、extractor-version cache invalidation 和索引持久化。多个文件在同一稳定性等待窗口内检查，而不是逐文件等待。来源 URL 先读 `com.apple.metadata:kMDItemWhereFroms` xattr，缺失时用 Spotlight 的 `MDItem` API（原生调用，不 fork `mdls`；子进程方式每次约 66 ms，60 个文件会拖慢一个数量级）。它只依赖 `ExtractorRegistry` 接口，不 import PDFKit 之外的具体格式库。
...
自定义扫描目录列表与整理目录通过 `PreferenceStore` 暴露给 CLI 和 GUI，默认扫描列表只有 Downloads。扫描目录必须真实存在、彼此不重叠，且与整理目录同卷；目录变化会关闭自动整理，需要再次启用。每个方案固化生成时的扫描目录及整理根目录，设置之后再变也不会改变该方案的移动、撤销路径。文件名冲突使用确定性后缀且绝不覆盖。

取消主题是持久操作：它把当前方案里该主题的成员标记为 `excluded`（因此无法被 apply 移动），同时为每个成员写入一条 `action='dismiss'` 的 correction。聚类在分组前读取每个文件最新的有效 correction：`exclude` 的进未分类，`dismiss` 的直接从本次建议中剔除（也不进未分类），所以下次扫描不会再把该主题提出来。界面把这些成员收进侧栏默认折叠的"已取消"分组；`restore-dismissed <显示名>` 会停用对应 correction，之后的 propose 会重新提出该主题。排除单个文件仍走 `exclude`，文件会回到未分类而不会消失。
...
发布流程是 `.github/workflows/release.yml`：push `v*` 标签 → 校验 tag 与版本 → **强制**导入签名证书（缺失即失败，避免某版悄悄变成 ad-hoc 签名）→ `swift test` → **唯一一次** release 编译 → 基准门禁 → `build_app.sh --skip-build` 与 `package_cli.sh --skip-build` 复用同一份二进制 → 生成 Release Notes 与 `SHA256SUMS.txt` → 校验产物签名 → 创建 GitHub Release → 后续 `tap` job 用 `publish_tap.sh` 把 Homebrew tap 更新到该版本。`tests.yml` 只在分支推送与 PR 上运行，忽略 `v*` 标签，避免一次发布跑两套相同测试。
